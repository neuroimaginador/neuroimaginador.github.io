...
Contribuciones en Github en el último año (actualizado a `r Sys.Date()`):
...
html <- xml2::read_html("https://github.com/neuroimaginador")
...
gh_data <- tibble(
  ...
  type = "github",
  tip = str_glue("{count} contribuciones"),
  link = str_glue("https://github.com/neuroimaginador?from={date}&to={date}&tab=overview")
)
```
...
pkgs <- get_pkgs("dominlopez@uma.es")
...
p_data <- filter(all_data, date > today() - years(1)) %>%
  mutate(
    ...
    tip = str_glue("<b>{tip}</b> el {date}")
  )
counts <- count(p_data, type, .drop = FALSE) %>%
  na.omit() %>%
  mutate(
    # n = case_when(
      # type == "github" ~ (count(p_data, type, wt = as.integer(count)) %>% na.omit() %>% pull()),
      # TRUE ~ n),
    emoji = c("💻", "📹", "📋", "📽", "⚒", "📦"),
...
Principales paquetes de R desarrollados:

- [wf4ni](/pkgs/wf4ni): Generación de _workflows_ específicos para Neuroimagen desde R, para favorecer la investigación reproducible.
- [fcaR](https://neuroimaginador.github.io/fcaR/): Herramientas para realizar análisis formal de conceptos (Formal Concept Analysis), trabajar con contextos formales, retículos de conceptos e implicaciones.
